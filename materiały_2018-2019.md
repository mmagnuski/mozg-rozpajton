...
Zauważcie - musi być ten dwukropek, który mówi pythonowi *rozpoczynam poniżej instrukcje do powtórzenia* oraz musi
być wcięcie dla instrukcji poniżej. To wcięcie to klasycznie tab albo 4 spacje, ale tak naprawdę możecie użyć
dowolnego wcięcia. Podobnie nie ma dużego znaczenia jak nazwiemy sobie to, co następuje tuż po `for` (u nas `x`) -
to po prostu taka umowa z pythonem *nazwijmy sobie kolejne wartości z listy wartości dodatnie jako x, teraz wykonaj:*
i w poniższych instrukcjach wiadomo będzie, że `x` to obecny element z `wartości_dodatnie`. Ale możemy nazwać go
sobie inaczej:
```python
